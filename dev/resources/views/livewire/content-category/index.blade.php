                            ...
                            <td>
                                <div class="flex justify-end">
                                    @can('content_category_show')
                                        <a class="btn btn-sm btn-info mr-2" href="{{ route('admin.content-categories.show', $contentCategory) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan
                                    @can('content_category_edit')
                                        <a class="btn btn-sm btn-success mr-2" href="{{ route('admin.content-categories.edit', $contentCategory) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan
                                    @can('content_category_delete')
                                        <button class="btn btn-sm btn-rose mr-2" type="button" wire:click="confirm('delete', {{ $contentCategory->id }})" wire:loading.attr="disabled">
                                            {{ trans('global.delete') }}
                                        </button>
                                    @endcan
                                </div>
